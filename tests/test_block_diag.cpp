...
block_diag_ilu::BlockDiagMatrix<double> get_test_case_colmajblockdiagmat(){
    constexpr int nblocks = 3;
    constexpr int blockw = 2;
    constexpr int ndiag = 1;
    constexpr int nsat = 0;
    constexpr int ld = 6;
    block_diag_ilu::BlockDiagMatrix<double> cmbdmv {nullptr, nblocks, blockw, ndiag, nsat, ld};
    // 5 3 2 # # #
    // 5 8 # 3 # #
    // 1 # 8 4 4 #
    // # 2 4 4 # 5
...
block_diag_ilu::BlockDiagMatrix<double> get_test_case_sat(){
    ...
    block_diag_ilu::BlockDiagMatrix<double> cmbdmv {nullptr, nblocks, blockw, ndiag, nsat};
    std::array<double, blockw*blockw*nblocks> blocks {{
            7, 6, 2, 6,
            1, 5, 3, 4,
            9, 2, 7, 3}};
...
TEST_CASE( "_get_test_m4 in test_fakelu.py", "[ILU]" ) {
    ...
    block_diag_ilu::BlockDiagMatrix<double> cmbdv {
...
TEST_CASE( "addressing", "[ColMajBlockDiagView]" ) {
    ...
    block_diag_ilu::BlockDiagMatrix<double> v {
        data.data(), 3, 2, 1, 0, 2};
...
TEST_CASE( "addressing multi diag", "[ColMajBlockDiagView]" ) {
    ...
    std::array<double, 2*3*3 + 3*(2+1) + 3*(2+1)> data {{
            ...
            17, 18, X, 19, 20, X,
                81, 82, X}};
    block_diag_ilu::BlockDiagMatrix<double> v {data.data(), 3, 2, 2, 0, 3};
...
TEST_CASE( "set_to_eye_plus_scaled_mtx", "[ColMajBlockDiagView]" ) {
    ...
    block_diag_ilu::BlockDiagMatrix<double> m {nullptr, nblocks, blockw, ndiag, nsat, ld};
    double gamma = 0.7;
...
TEST_CASE( "long_double_ilu_inplace", "[ILU_inplace]" ) {
    ...
    constexpr int ld = 2;
    block_diag_ilu::BlockDiagMatrix<long double> cmbdmv {nullptr, nblocks, blockw, ndiag, nsat, ld};
    // 5 3 2 # # #
    // 5 8 # 3 # #
    // 1 # 8 4 4 #
    // # 2 4 4 # 5
...
TEST_CASE( "ilu inplace with blockw1", "[ILU_inplace]" ) {
    ...
    //
    // y[1]/4 + y[2]/2 + y[3] = 65
    //     y[3] = 65 - 137/4 + 2/2 = 31.75
    //
    // 3*y[2]/8 + y[4] = 60
    //     y[4] = 60 - 3*(-2)/8 = 60.75
    //
    // y[3] + y[4]/3 + y[5] = 121
    //     y[5] = 121 - 31.75 - 60.75/3 = 69
    //
    // 7*x[5] = 69
    //     x[5] = 69/7 = 9.857142857142858
    // 6*x[4] + 9*x[5] = 60.75
    //     x[4] = (60.75 - 9*x[5])/6 = -4.660714285714287
    // 4*x[3] + 5*x[5] = 31.75
    //     x[3] = (31.75 - 5*x[5])/4 = -4.383928571428573
    // 8*x[2] + 4*x[3] + 4*x[4] = -2
    //     x[2] = (-2 - 4*x[3] - 4*x[4])/8 = 4.272321428571431
    // 8*x[1] + 3*x[3] = 137
    //     x[1] = (137 - 3*x[3])/8 = 18.768973214285715
    // 5*x[0] + 3*x[1] + 2*x[2] = 65
    //     x[0] = (65 - 3*x[1] - 2*x[2])/5 = 0.02968749999999858


    SECTION( "solve performs adequately" ) {
        std::array<double, 6> b {{65, 202, 11, 65, 60, 121}};
        std::array<double, 6> xref {{0.02968749999999858,
                    18.768973214285715,
                    4.272321428571431,
                    -4.383928571428573,
                    -4.660714285714287,
